...
int diff_pdf(int argc, char *argv[])
{
    wxAppConsole::CheckBuildOptions(WX_BUILD_OPTIONS_SIGNATURE, "diff-pdf");
    wxInitializer wxinitializer(argc, argv);
...
extern "C" {
    __declspec(dllexport) int export_diff_pdf(char* file_path1, char* file_path2, char* outpath)
    {
        g_verbose = true;
        g_mark_differences = true;

        wxFileName file1(file_path1);
        wxFileName file2(file_path2);
        file1.MakeAbsolute();
        file2.MakeAbsolute();
        const wxString url1 = wxFileSystem::FileNameToURL(file1);
        const wxString url2 = wxFileSystem::FileNameToURL(file2);
        ...
        if ( !doc1 )
        {
            fprintf(stderr, "Error opening %s: %s\n", (const char*) file_path1, err->message);
            g_error_free(err);
            return 3;
        }
